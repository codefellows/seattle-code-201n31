...
function Kitten(name,age,interests, isGoodWithKids, isGoodWithDogs,isGoodWithCats,image){
  this.name = name;
  this.age = age;
...
Kitten.prototype.render = function(){
  ...
  const kittenArticle = document.querySelector(`#${this.name}`);

  // console.log(tableRow);
  const newTableRow = tableRow.cloneNode(true);
  kittenArticle.append(newTableRow);
};
...
function handleSubmit(event) {
  ...
  let kittenAge = event.target.number.value;
  let kittenImage = event.target.photo.value;
  ...
  let interestsArray = event.target.interests.value.split(',');

  const userKitten = new Kitten(kittenName, kittenAge, interestsArray, isGoodWithKids, isGoodWithDogs, isGoodWithCats, kittenImage)
